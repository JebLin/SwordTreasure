...
/**
 * Created by rd_jianbin_lin on 2017/9/5. 1. 什么是Fork/Join框架
 * Fork/Join框架是Java7提供了的一个用于并行执行任务的框架， 是一个把大任务分割成若干个小任务， 最终汇总每个小任务结果后得到大任务结果的框架。
 *
 * 我们再通过Fork和Join这两个单词来理解下Fork/Join框架， Fork就是把一个大任务切分为若干子任务并行的执行，
 * Join就是合并这些子任务的执行结果，最后得到这个大任务的结果。
 * 比如计算1+2+。。＋10000，可以分割成10个子任务，每个子任务分别对1000个数进行求和，最终汇总这10个子任务的结果。
 *
 * 2. 工作窃取算法 工作窃取（work-stealing）算法是指某个线程从其他队列里窃取任务来执行。
 *
 ...
 *
 * Fork/Join使用两个类来完成以上两件事情： ForkJoinTask：我们要使用ForkJoin框架，必须首先创建一个ForkJoin任务。
 * 它提供在任务中执行fork()和join()操作的机制，通常情况下我们不需要直接继承ForkJoinTask类，
 * 而只需要继承它的子类，Fork/Join框架提供了以下两个子类： RecursiveAction：用于没有返回结果的任务。 RecursiveTask
 * ：用于有返回结果的任务。
 *
 ...
 */
public class _19_TestForkJoinPool_RecursiveTask {
	public static void main(String[] args) {
		Instant start = Instant.now();
		ForkJoinPool pool = new ForkJoinPool();
		ForkJoinTask<Long> task = new ForkJoin_RecursiveTask(0L, 5000000000l);
...
class ForkJoin_RecursiveTask extends RecursiveTask<Long>{
	...
	public ForkJoin_RecursiveTask(long start, long end) {
		this.start = start;
		this.end = end;
	}
	...
	protected Long compute() {
		...
		if(length <= THURSHOLD){
			...
			ForkJoin_RecursiveTask left = new ForkJoin_RecursiveTask(start, middle); 
			left.fork(); //进行拆分，同时压入线程队列
			
			ForkJoin_RecursiveTask right = new ForkJoin_RecursiveTask(middle+1, end);
			right.fork(); //
